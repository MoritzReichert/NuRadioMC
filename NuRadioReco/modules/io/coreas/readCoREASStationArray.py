            ...
            for iCore, core in enumerate(cores):
                ...
                for station_id in station_ids:
                    ...
                    if(dcore > ddmax):
                        ...
                        sim_station = coreas.make_sim_station(station_id, corsika, None, channel_ids, interpFlag=self.__perform_interpolation)
                        ...
                        station = NuRadioReco.framework.station.Station(station_id)
                        channel_ids = detector.get_channel_ids(station_id)
                        sim_station = coreas.make_sim_station(station_id, corsika, observer, channel_ids, interpFlag =self.__perform_interpolation)
                        station.set_sim_station(sim_station)
                        evt.set_station(station)
                        ...
                        for i, observer in enumerate(corsika['CoREAS']['observers'].values()):
                            ...
                            signal = (np.array([electric_field[:,0]*units.second, -electric_field[:,2]*conversion_fieldstrength_cgs_to_SI, electric_field[:,1]*conversion_fieldstrength_cgs_to_SI, electric_field[:,3]*conversion_fieldstrength_cgs_to_SI]).T)
                        ...
                        signal_interpolator = cr_pulse_interpolator.signal_interpolation_fourier.interp2d_signal(positions_vBvvB[:,0], positions_vBvvB[:,1], 
